$(document).ready(function () {
  ...
  async function drawMap() {
    const myTopoJson = await d3
      .json(
        "https://raw.githubusercontent.com/ValeriiaShur/geo-data/master/de_5_staged.json"
      )
      .catch((err) => {
        console.error(err);
      });
    ...
    const state_features = topojson.feature(
      myTopoJson,
      myTopoJson.objects.de_5_staged
    ).features;
    ...
    choroPoly
      ...
      .attr("d", function (d, i) {
        bPathsStart[i] = SVGTag2Path.Rect(
          ...
          xScale.bandwidth() / 10,
          (height + margin.top - yScale(d.properties.value)) / 10
        );
    ...
    addButton("Tween to Bar Chart", function (d, i) {
      ...
      // polyLayer.transition().duration(100).style("opacity", 0);
