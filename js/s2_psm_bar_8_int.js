$(document).ready(function () {
  ...
  async function drawMap() {
    const myTopoJson = await d3
      .json(
        "https://raw.githubusercontent.com/ValeriiaShur/geo-data/master/nl_8_interpolated_s2.json"
      )
      .catch((err) => {
        console.error(err);
      });
    ...
    const state_features = topojson.feature(
      myTopoJson,
      myTopoJson.objects.nl_8_interpolated_s2
    ).features;
